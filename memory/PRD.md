...
## Latest (Feb 2026): **Electric guitar v3 — Yamaha MoxF6 "Hard Rock AS1&2"-style synth voice with press/release sustain** — Completely rewrote `components/GuitarInstrument.js`. Ditched Tone.js real samples (v2 sounded generic and wouldn't sustain) and Karplus-Strong synth (v1 too bright/loud) in favor of a **detuned sawtooth + sub-octave square + envelope-modulated LP + tame WaveShaper distortion** voice architecture — the classic hard-rock analog synth patch. Each note is a persistent Voice object with its own amp ADSR (A=6ms D=90ms S=0.65 R=260ms) and LP filter envelope (1200→4500→sustain, Q=3.5 for wah-pedal-half-open signature). Multiple voices held simultaneously via `activeVoicesRef` Map. Master post-gain 0.11-0.13 to match bells/xylo/piano loudness. **Press/release sustain**: `onPointerDown`/`keydown` starts voice, `onPointerUp`/`onPointerLeave`/`onPointerCancel`/`keyup` releases it — notes ring for as long as held, decay musically on lift. Visual "held" state (scale 0.96, y+3) mirrors physical press for as long as sustain lasts. Cleanup releases all voices on unmount. **Fixed keyboard conflict**: `FreePlayPage.js` line 687+ now short-circuits its keydown/keyup listeners when `activeTab === 'guitar'` so bell/piano/xylo tabs' A-L key mappings no longer double-fire with guitar's mappings.
