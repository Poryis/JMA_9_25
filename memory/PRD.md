...
## Latest (Feb 2026): **Electric guitar v6 — visual strings, power chords for crunch, volume up** — On top of v5's FluidR3 real-sample engine: (1) **Guitar visual**: SVG guitar surface at the top of the component with 6 real strings (thickness varies low E chunky → high E thin, bronze on low 2 / steel on top 4), tuning pegs on the left, black pickup bar with 6 pole-pieces in the middle, bridge saddles on the right. Each string vibrates independently via framer-motion path animation (5-frame keyframe wobble over 0.9s) when its note plays. Chord strums vibrate strings in sequence matching the 22ms per-string offset — kids see + hear a real down-strum. Notes-mode plays vibrate the string that corresponds to the pentatonic pad. (2) **Power chords on Crunch**: full 6-note voicings become muddy under distortion, so we automatically swap to 3-note power chords (root + 5th + octave) when tone === 'crunch'. Chord label shows `Am5` instead of `Am` on crunch, string-count dots on each pad show 3 vs 6, and a red "POWER CHORDS" badge sits on the guitar visual. (3) **Volume**: chord baseLevel 0.35 → **0.60**, note level 0.55 → **0.90**. Guitar now sits center-stage when active, still not louder than bells/drums combined. — Power chord voicings: Am5 [A2 E3 A3], Dm5 [D3 A3 D4], Em5 [E2 B2 E3], G5 [G2 D3 G3], C5 [C3 G3 C4], F5 [F2 C3 F3].
