...
## Latest (Feb 2026): **Home JMAtv card — logo stacks above the CRT on phones** — `components/RetroTV.js` swapped the mobile `order-*` values on the two flex children so when the card collapses to vertical (< md), the JMAtv logo + "Watch today's episode!" tagline sit ABOVE the CRT TV (order-1) and the CRT drops below (order-2). Desktop layout unchanged: logo LEFT (order-1) → CRT CENTER (order-2) → sunburst RIGHT (order-3). Verified: phone 390×844 `logo_y=376 < tv_y=565`, desktop 1400×900 `logo_x=323 < tv_x=595` (side-by-side preserved).
