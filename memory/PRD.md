...
## Latest (Feb 2026): **Electric guitar v5 — real FluidR3 GM samples, chromatic, press/release gated** — Replaced the synth-voice engine entirely. Downloaded **27 chromatic samples per tone** (C3-E5, both `electric_guitar_clean` and `distortion_guitar` from gleitz/midi-js-soundfonts, MIT) to `/public/assets/audio/guitar-clean/` and `/public/assets/audio/guitar-crunch/`. Real recorded electric guitar → real pick attack, real body. Full chromatic coverage means pitch-shift is at most a half-semitone (inaudible). `Voice` object wraps a `BufferSource` pitch-shifted to exact target freq via `playbackRate = target/sampleFreq`. **Press/release gating**: samples decay naturally (~2s like a real guitar); release fires an exponential fade to silence over 120 ms (palm-mute feel) plus a hard `source.stop()` a few frames later. Chord = 6 layered voices with 22 ms strum offsets. Levels calibrated: 0.35 base for chord strings (× decay across strings) + 0.55 for pentatonic notes → matches bells/xylo/piano loudness. Both tones preload in parallel on mount ("loading clean tone… / loading crunch tone…" pill). Keyboard bindings unchanged (1-6 chords, A S D F G H J K L pentatonic, Q/W tone, Z/X mode). v1-v4 (Karplus-Strong, Tone.js sparse pack, synth voice, synth+pick transient) all deprecated — user rejected each: too synthy, generic, no sustain, unrealistic attack.
