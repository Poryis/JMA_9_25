...
## Latest (Feb 2026): **GarageBand-style electric guitar — Jam Session Stage 2 (real samples)** — Rewrote `components/GuitarInstrument.js` to use **real sampled electric guitar** (Tone.js instruments pack, MIT-licensed, 11 × ~100 KB MP3s at `/public/assets/audio/guitar/`: A2/3/4/5, C3/4/5, E2, Fs3/4/5). Sparse sample set filled by playbackRate pitch-shifting to nearest semi-tone. Master gain slashed 0.55 → **0.18** to match bells/xylo/piano loudness. Chord strum layer gain 0.16 (was 0.45). Crunch tone tamed (WaveShaper k=100 vs 400, post-gain 0.35). **Keyboard bindings** added: 1–6 → Am/Dm/Em/G/C/F, A/S/D/F/G/H/J/K/L → 9 pentatonic notes, Q/W → Clean/Crunch, Z/X → Chords/Notes mode. All active only while the Guitar tab is mounted. On-screen key hints rendered on each pad. Samples preload on tab open with a "loading samples…" pill. **Prev Stage 1 (Karplus-Strong synth)** replaced — user rejected tone quality and reported volume was 3× too loud.
